...
function XemDanhGiaContent() {
    ...
    // notCompletedUsers: Người chưa đánh giá ai cả (0 đánh giá)
    const notCompletedUsers = targetUsers.filter(u => {
        ...
        // User hasn't evaluated anyone at all
        return userEvaluations.length === 0;
    });

    // inprogressUsers: Người đã đánh giá nhưng chưa hoàn thành (đã đánh giá ít nhất 1, nhưng chưa đánh giá hết)
    const inprogressUsers = targetUsers.filter(u => {
        // Get all members in the same boPhan (department section) that need to be evaluated
        const departmentMembers = users.filter(member =>
            member.phongBanId === u.phongBanId &&
            member.boPhan === u.boPhan && // Same boPhan (department section)
        ...
        // Check if user has evaluated at least one but not ALL department members
        const evaluatedMemberIds = new Set(userEvaluations.map(dg => dg.nguoiDuocDanhGiaId));
        const hasEvaluatedSome = userEvaluations.length > 0;
        const hasEvaluatedAll = departmentMembers.every(member => evaluatedMemberIds.has(member.id));

        return hasEvaluatedSome && !hasEvaluatedAll;
    });
                ...
                <Paper withBorder shadow="sm" p={isMobile ? "sm" : "md"} radius="md">
                    <Stack gap="sm">
                        ...
                        <Group gap="xl" wrap="wrap">
                            ...
                            <div>
                                <Text size="sm" c="dimmed">Đang tiến hành</Text>
                        ...
                        {inprogressUsers.length > 0 && (
                            <>
                                <Divider />
                                <div>
                                    <Text fw={600} mb="xs">Danh sách đang tiến hành:</Text>
                                    <Group gap="xs" style={{ maxHeight: '150px', overflowY: 'auto' }}>
                                        {inprogressUsers.map(user => (
                                            <Badge key={user.id} color="orange" variant="light">
                                                {user.hoTen} ({user.maNhanVien})
                                            </Badge>
                                        ))}
                                    </Group>
